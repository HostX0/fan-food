...
export default class MenuApp extends React.Component<Record<string, never>, State> {
    state: State = { cart: [], ready: false, category: 'all', query: '', sort: 'default', favorites: [], selected: null, variant: '', choice: '', itemNote: '', quantity: 1, cartOpen: false, stage: 'cart', customer: { ...emptyCustomer }, errors: {}, reference: '', toast: '', confirmClear: false, copied: false };
    ...
    goMenu = (category = 'all') => { this.setState({ category }, () => { document.getElementById('menu')?.scrollIntoView({ behavior: matchMedia('(prefers-reduced-motion: reduce)').matches ? 'auto' : 'smooth', block: 'start' }); }); };
    ...
    quickAdd = (product: Product) => { if (product.variants.length > 1 || product.choices.length)
        this.openProduct(product);
    else
        this.add(product, product.variants[0].id, 1, '', ''); };
    add = (product: Product, variant: string, quantity: number, choice: string, note: string) => {
        this.setState(s => ({ cart: addLine(s.cart, product, variant, quantity, choice, note), selected: null }), () => this.notify('انضاف ' + product.name + ' للسلة'));
    };
   ...
   <div className="product-body"><button className="product-title" onClick={() => this.openProduct(product)}><h3>{product.name}</h3></button><p>{product.variants.length > 1 ? `${product.variants.length} أحجام للاختيار` : product.variants[0].label === 'الطبق' ? 'من مطبخ فن فود' : product.variants[0].label}{product.choices.length > 0 ? ' • لحم أو دجاج' : ''}</p>
    <div className="product-bottom"><Price value={amount} from={product.variants.length > 1}/><button type="button" className="add-button" aria-label={'أضف ' + product.name} onClick={() => this.quickAdd(product)}><Icon name="plus" size={13}/><span>{product.variants.length > 1 || product.choices.length ? 'اختار' : 'أضف'}</span></button></div>
   </div>
  </article>;
    };
   ...
   <img src={line.product.image} width="64" height="64" alt="" loading="lazy"/><div className="cart-line-info"><h3>{line.product.name}</h3><p>{line.variant.label}{line.choice ? ' • ' + line.choice : ''}</p>{line.note && <p className="line-note">{line.note}</p>}<Price value={line.total}/><Quantity value={line.quantity} label={'كمية ' + line.product.name} onChange={n => this.updateQuantity(line.key, n)}/></div>
   <button className="icon-button remove-button" type="button" aria-label={'حذف ' + line.product.name} onClick={() => this.remove(line.key)}><Icon name="trash" size={13}/></button>
   ...
   <div className="drawer-heading"><div><span className="eyebrow">خطوة أقرب للّمة</span><h2 id="cart-dialog-title" tabIndex={-1} data-dialog-heading>{stage === 'cart' ? 'طلبك، على ذوقك.' : stage === 'details' ? 'وين نوصل طلبك؟' : 'رسالتك صارت جاهزة.'}</h2></div><button className="icon-button" aria-label="إغلاق السلة" onClick={() => this.setState({ cartOpen: false })}><Icon name="close"/></button></div>
   ...
   {!count ? <div className="drawer-body"><EmptyState title="سلتك تنتظر أكلاتك"><p>تصفّح المنيو وضيف اللي تشتهيه.</p><button className="button button-primary" onClick={() => this.setState({ cartOpen: false }, () => this.goMenu())}>اكتشف المنيو<Icon name="arrow" size={14}/></button></EmptyState></div> : stage === 'cart' ? <>
     <div className="drawer-body">{this.renderCartLines()}<button className="text-link continue-shopping" onClick={() => this.setState({ cartOpen: false })}><Icon name="plus" size={13}/>ضيف شي ثاني للّمة</button>
   ...
   <main>
    ...
    <section className="menu-section container" id="menu" aria-labelledby="menu-title"><div className="menu-heading"><div><span className="eyebrow">على سفرتكم، من فن فود</span><h2 id="menu-title">اختار اللي <span>يطيّب خاطرك.</span></h2><p>أكلات عراقية، معجنات، وحلو يكمّل اللمة.</p></div><div className="menu-tools"><div className="search-box"><Icon name="search" size={18}/><input type="search" aria-label="ابحث في المنيو" placeholder="اسم الأكلة، نوعها، أو ميزانيتك…" maxLength={100} value={query} onChange={e => this.setState({ query: e.target.value, category: 'all' })}/>{query && <button aria-label="مسح البحث" className="icon-button" onClick={() => this.setState({ query: '' })}><Icon name="close" size={13}/></button>}</div><div className="sort-box"><Icon name="sort" size={13}/><select aria-label="ترتيب الأصناف" value={sort} onChange={e => this.setState({ sort: e.target.value })}><option value="default">ترتيب المنيو</option><option value="low">السعر: من الأقل</option><option value="high">السعر: من الأعلى</option></select></div></div></div>
     ...
     <div className="menu-layout"><div className="menu-content">
      ...
      {!visible.length ? <EmptyState icon={category === 'favorites' ? 'heart' : 'search'} title={category === 'favorites' ? 'خلّي مفضّلاتك قريبة' : 'ما لكينا هالأكلة'}><p>{category === 'favorites' ? 'اضغط القلب على أي صنف حتى ترجعله بسهولة.' : 'جرّب اسم ثاني، أو ابحث بكل الأقسام.'}</p><button className="button button-outline" onClick={() => this.setState({ category: 'all', query: '' })}>عرض كل المنيو</button></EmptyState> : grouped ? categories.map(c => <section className="category-section" key={c.id} aria-labelledby={'heading-' + c.id}><div className="section-heading"><div><span className="section-icon"><Icon name={c.icon} size={18}/></span><div><h3 id={'heading-' + c.id}>{c.name}</h3><p>{c.description}</p></div></div><span className="section-count">{products.filter(p => p.categoryId === c.id).length} أصناف</span></div><div className="product-grid">{visible.filter(p => p.categoryId === c.id).map(this.renderProduct)}</div></section>) : <>
    ...
    <section className="how-section container" id="how-to-order"><div className="how-title"><span className="eyebrow">بخطوات بسيطة</span><h2>من تختار…<br />إلى ما ترسل طلبك.</h2></div><div className="how-steps"><article><span>01</span><Icon name="all" size={25}/><h3>اختار أكلاتك</h3><p>تصفّح الأقسام وحدد الحجم والعدد.</p></article><article><span>02</span><Icon name="bag" size={25}/><h3>رتّب سفرتك</h3><p>راجع السلة وضيف بيانات التوصيل.</p></article><article><span>03</span><Icon name="whatsapp" size={27}/><h3>دزّها واتساب</h3><p>افتح الرسالة واضغط إرسال، ونأكد التفاصيل وياك.</p></article></div></section>
   </main>
   <footer className="site-footer"><div className="container footer-main"><div className="footer-brand"><img src="/images/logo-light.png" alt="فن فود" width="155" height="86"/><p>من مطبخنا إلى لمّتكم.</p></div><div><h3>خلّينا على تواصل</h3><a href={whatsappUrl(SITE.whatsapp)} target="_blank" rel="noopener noreferrer"><Icon name="whatsapp"/><span dir="ltr">{SITE.phoneDisplay}</span></a><a href={SITE.instagram} target="_blank" rel="noopener noreferrer"><Icon name="instagram"/><span dir="ltr">@Fanfood.iq</span></a></div><div className="footer-note"><Icon name="leaf" size={22}/><p>اللمة أحلى<br />لما يكون الأكل من البيت.</p></div></div><div className="container footer-bottom"><span>فن فود • أكل البيت، بروح اليوم.</span><a href={SITE.agencyUrl} target="_blank" rel="noopener noreferrer" dir="ltr">{SITE.agencyName}<Icon name="arrow" size={12}/></a><button className="back-to-top" aria-label="الرجوع للأعلى" onClick={() => window.scrollTo({ top: 0, behavior: matchMedia('(prefers-reduced-motion: reduce)').matches ? 'auto' : 'smooth' })}><Icon name="up" size={13}/></button></div></footer>
