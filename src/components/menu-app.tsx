...
export default class MenuApp extends React.Component<Record<string, never>, State> {
    ...
    add = (product: Product, variant: string, quantity: number, choice: string, note: string) => {
        this.setState(s => ({ cart: addLine(s.cart, product, variant, quantity, choice, note), selected: null }), () => this.notify('أُضيف للسلة'));
    };
   ...
   <div className="product-body"><button className="product-title" onClick={() => this.openProduct(product)}><h3>{product.name}</h3></button><p>{product.variants.length > 1 ? `${product.variants.length} أحجام` : product.variants[0].label === 'الطبق' ? 'جاهز للطلب' : product.variants[0].label}{product.choices.length > 0 ? ' • اختيارات متاحة' : ''}</p>
   ...
   <img src={line.product.image} width="64" height="64" alt="" loading="lazy"/><div className="cart-line-info"><h3>{line.product.name}</h3><p>{line.variant.label}{line.choice ? ' • ' + line.choice : ''}</p>{line.note && <p className="line-note">{line.note}</p>}<Price value={line.total}/>{this.renderStepper(line.quantity, () => this.incrementLine(line.key), () => this.decrementLine(line.key), 'كمية ' + line.product.name, 'cart-stepper')}</div>
   ...
   <main>
    ...
    <section className="menu-section container" id="menu" aria-labelledby="menu-title"><div className="menu-heading"><div><span className="eyebrow">على سفرتكم، من فن فود</span><h2 id="menu-title">اختار اللي <span>يطيّب خاطرك.</span></h2></div><div className="menu-tools"><div className="search-box"><Icon name="search" size={18}/><input type="search" aria-label="ابحث في المنيو" placeholder="اسم الأكلة، نوعها، أو ميزانيتك…" maxLength={100} value={query} onChange={e => this.setState({ query: e.target.value, category: 'all' })}/>{query && <button aria-label="مسح البحث" className="icon-button" onClick={() => this.setState({ query: '' })}><Icon name="close" size={13}/></button>}</div><div className="sort-box"><Icon name="sort" size={13}/><select aria-label="ترتيب الأصناف" value={sort} onChange={e => this.setState({ sort: e.target.value })}><option value="default">ترتيب المنيو</option><option value="low">السعر: من الأقل</option><option value="high">السعر: من الأعلى</option></select></div></div></div>
     ...
     <div className="menu-layout"><div className="menu-content">
      ...
      <div className="catalog-footnote"><Icon name="info" size={15}/><p>الأسعار بالدينار العراقي. بعض الصور توضيحية؛ التوصيل والتوفّر يُؤكّدان عبر واتساب.</p></div>
     </div>{this.renderCartSummary()}</div>
    </section>
    <section className="story-section" id="our-story"><div className="container story-inner"><div className="story-photo"><img src="/images/packaging.webp" alt="تغليف فن فود" width="883" height="669" loading="lazy"/></div><div className="story-copy"><span className="eyebrow"><Icon name="leaf" size={15}/>عن فن فود</span><h2>أكل بيت،<br /><span>بلمسة مرتبة.</span></h2><p>أكلات عراقية محضّرة للّمة، بطلب سهل وسريع.</p><div className="story-signature">أكل البيت، بروح اليوم.</div><a href={SITE.instagram} className="text-link" target="_blank" rel="noopener noreferrer"><Icon name="instagram" size={19}/>شوف أكثر<Icon name="arrow" size={13}/></a></div></div></section>
    <section className="how-section container" id="how-to-order"><div className="how-title"><span className="eyebrow">3 خطوات</span><h2>اختيار، مراجعة،<br />ثم إرسال.</h2></div><div className="how-steps"><article><span>01</span><Icon name="all" size={25}/><h3>اختار</h3><p>حدد الصنف والكمية.</p></article><article><span>02</span><Icon name="bag" size={25}/><h3>راجع</h3><p>رتب السلة بسهولة.</p></article><article><span>03</span><Icon name="whatsapp" size={27}/><h3>أرسل</h3><p>يفتح الطلب مباشرة في واتساب.</p></article></div></section>
   </main>
